                ...
                <div class="row">
                    <div class="input-field col s12 l6">
                        <select name="category">
                            <option value="" selected disabled>{{ __('Choose Your Option') }}</option>
                            <option value="{{ __('About Services') }}" @if (old('subjects') == __('About Services')) selected @endif>{{ __('About Services') }}</option>
                            <option value="{{ __('About Sites') }}" @if (old('subjects') == __('About Sites')) selected @endif>{{ __('About Sites') }}</option>
                            <option value="{{ __('Report Bugs, Typos') }}" @if (old('subjects') == __('Report Bugs, Typos')) selected @endif>{{ __('Report Bugs, Typos') }}</option>
                            <option value="{{ __('Report Users') }}" @if (old('subjects') == __('Report Users')) selected @endif>{{ __('Report Users') }}</option>
                            <option value="{{ __('Report Posts') }}" @if (old('subjects') == __('Report Posts')) selected @endif>{{ __('Report Posts') }}</option>
                            <option value="{{ __('Other') }}" @if (old('subjects') == __('Other')) selected @endif>{{ __('Other') }}</option>
                        </select>
                        <label>{{ __('Choose Subjects') }}</label>
                    </div>
                    <div class="input-field col s12">
                        <label for="email">{{ __('E-Mail Address') }}</label>
                        <input id="email" type="email" class="validate" name="email" value="{{ $user->email ?? old('email') }}" required autocomplete="email">
                    </div>
                    <div class="input-field col s12">
